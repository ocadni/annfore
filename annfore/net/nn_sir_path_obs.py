...
class SIRPathColdObs(common_net.Autoreg):
    ...
    def __init__(self, neighs, T:int,
                obs_list:list,
        ...
        for i in range(self.num_nets):
            self.params_i[i] = tuple( filter(lambda p: p.requires_grad,
                              self.sublayers[i].parameters()) )
        ...
        for lay in self.sublayers:
            for mskdeep in lay:
                ...
                for mod in mskdeep.net:
                    deep_linear.reset_weights_on_net(mod, lin_r=lin_r, bias_r=bias_r, kind=method)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_nets):
            ...
            t_rec_probs = self._get_trec_probs(i, samples_select, sam_indics, batch_size)
    ...
    def log_prob_i(self, node_i, samples):
        ...
        t_rec_probs = self._get_trec_probs(node_i, samples_select, sam_indics, batch_size)

        sam_indics = samples[:,indix,1] - self.sublayers[node_i][1].index_out[0]
        probs_r_sel = t_rec_probs[bselect, sam_indics]

        log_prob_i = torch.log(probs_i_sel.clamp(min=self.min_value_prob)) + \
            torch.log(probs_r_sel.clamp(min=self.min_value_prob))
